...
var NOT_RETRYABLE = 1;
var RETRYABLE = 2;
var FIVE_SECONDS = moment.duration(5, 'seconds');
var RETRYABLE_ERROR_CODES = ['ETIMEDOUT'];
var LOGIN_HANDLERS = {
      url : function(options, callback) {
        request(options.loginUrl, {
          ...
          timeout: options.timeout || FIVE_SECONDS
        }, function(err, response) {
          if (err != null && RETRYABLE_ERROR_CODES.indexOf(err.code) >= 0) {
            callback(err, RETRYABLE);
          } else if (err != null) {
...
RequestHandler.prototype.login = function(callback) {
  ...
  this.options.logger.info('attempting to log in; attempt: {0}', (self.loginRetryCount + 1));
  loginHandler(this.options, function(err, retry) {
    if (err != null && retry === RETRYABLE) {
      self.loginRetryCount++;
      self.options.logger.error('login failed, reason: {0}', err.message);
      self.login(callback);
    } else if (err != null) {
      callback(err);
    } else {
